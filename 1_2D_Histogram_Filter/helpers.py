...
def normalize(grid):
    ...
    """
    #get the sum of beliefs
    total = 0.0
    for row in grid:
        for cell in row:
            total += cell
    #go through beliefs and update its normalize belief
...
def blur(grid, blurring):
    ...
    for i in range(height):
        for j in range(width):
            #confirm a position i,j, take is as the center point
            grid_val = grid[i][j]
            #base on the position i,j, blur all the 9 positions
            for dx in range(-1,2):
                for dy in range(-1,2):
                    #confirm the blur constant 'mult'
                    mult = window[dx+1][dy+1]
                    #confirm the blur position
                    new_i = (i + dy) % height
                    new_j = (j + dx) % width
                    #base on the i,j position, blur operation
                    new[new_i][new_j] += mult * grid_val
    #normalize the blurred beliefs
